import { getRecMarketplaceContractInstance, getRoleJsonKey } from '../utils/web3-utils';
...
export const constructRolesTable = async (prisma: PrismaClient) => {
    ...
    await seedRoles(prisma, parseInt(utils.ethereumBlockHeight, 10)).catch((err: Error) => {
        console.error(`error when seeding roles from block ${utils.ethereumBlockHeight}: ${err.message}`);
    });
};
